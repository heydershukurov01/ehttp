...
@Injectable()
export class POST extends HttpSandbox implements Request {
  ...
  */
  ...
  public setParams(params: any): this {
    if (params.base64) {
      ...
      if (base64Parameter.length > 1) {
        console.warn('[POST] [setParams] First dimension parameter must be single !');
      }
  ...
  public request(parameter: string | number = ''): Observable<any> {
    const request = this._http.post(super.getApi() + parameter, super.getPayload(), this.options());
    this.initialize();
    return request;
  }
}
